...
})({"dkIKo":[function(require,module,exports) {
var global = arguments[3];
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "b374353e6887d6c8";
"use strict";
...
var Particles = /** @class */ function() {
    ...
    /** START *************************/ Particles.prototype.start = function() {
        ...
        this.interval = setInterval(function() {
            ...
            if (nbParticles >= _this.settings.maxParticles) return;
            // Clone svg
            var svgClone = _this.svg.cloneNode(true);
            var path = svgClone.querySelector("path");
            if (path == null) {
                _this.stop();
                return alert("ERROR SVG\n<path> is require in the SVG");
            }
            // Scale
            var width = _this.randomRange(_this.settings.minWidth, _this.settings.maxWidth);
            var height = width * _this.aspectRatio;
            _this.svg.style.width = width + "px";
            _this.svg.style.height = height + "px";
            // Color
            var r = _this.randomRange(_this.settings.r - 5, _this.settings.r + 5);
            var g = _this.randomRange(_this.settings.g - 5, _this.settings.g + 5);
            var b = _this.randomRange(_this.settings.b - 5, _this.settings.b + 5);
            var a = _this.randomRange(_this.settings.minOpacity, _this.settings.maxOpacity);
            var color = "rgba(".concat(r, ", ").concat(g, ", ").concat(b, ", ").concat(a, ")");
            path.style.fill = color;
            // Spawn
            var spawnX = _this.randomRange(0, 100);
            var spawnY = _this.randomRange(0, 100);
            svgClone.style.top = spawnY + "%";
            svgClone.style.left = spawnX + "%";
            // Rotation
            var rotation = _this.randomRange(0, 360);
            // Movement
            var speed = _this.randomRange(_this.settings.minSpeed, _this.settings.maxSpeed);
            var translateX = _this.oppositeChance(Math.floor(_this.randomRange(0, speed)));
            var translateY = _this.oppositeChance(Math.floor(Math.sqrt(Math.pow(speed, 2) - Math.pow(translateX, 2))));
            var duration = _this.randomRange(_this.settings.minDuration, _this.settings.maxDuration);
            svgClone.animate([
                {
                ...
                {
                    transform: "translate(calc(-50% + ".concat(translateX, "px), calc(-50% + ").concat(translateY, "px)) rotate(").concat(rotation, "deg)"),
                    opacity: "0"
                }
            ], {
                duration: duration,
            ...
            // Create the particle
            var particle = _this.container.appendChild(svgClone);
            nbParticles++;
            // Destroy
            setTimeout(function() {
                // Remove particle
